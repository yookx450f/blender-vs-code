...
def setup_cut2_transparency_start(camera, car_a, car_b, center_pos_a, center_pos_b, separated_pos_a, separated_pos_b, cam_fixed):
    ...
    カメラ位置: X=1m固定、Y:-8→-9渐变、Z:0.5→7渐变（中間点経由）。
    3段階キーフレーム: (1,-8,0.5) → (1,-8.5,2.75) → (1,-9,7)
    """
    print("\n  === カット1-2: 半透明化 + Y:-8→-9渐变, Z:0.5→7渐变 (AUTO_CLAMPED) ===")
    ...
    # カメラ位置: X=1固定、Y:-8→-9渐变、Z:0.5→7渐变（AUTO_CLAMPEDが自動イージング）
    # 3段階キーフレーム設定
    cam_start = (cam_fixed[0], -8.0, 0.5)              # fr72: (1, -8, 0.5)
    cam_middle = (cam_fixed[0], -8.5, 2.75)            # fr144: (1, -8.5, 2.75) - 中間点
    cam_end = (cam_fixed[0], -9.0, 7.0)                # fr216: (1, -9, 7.0)

    middle_frame = int((CUT2_START + CUT2_END) / 2)     # fr144
    ...
    print(f"  [フレーム {CUT2_START}-{CUT2_END}] カメラ: (1,-8,0.5)→(1,-8.5,2.75)→(1,-9,7) (AUTO_CLAMPED自動イージング)")
    print(f"  [フレーム {CUT2_END}] carA={center_pos_a}, carB={center_pos_b} (重叠維持)")
...
def setup_cut3_separation(camera, car_a, car_b, center_pos_a, center_pos_b, separated_pos_a, separated_pos_b, cam_fixed, cam_height):
    ...
    fr360終了時、カメラ位置は (1, -9, 1.0) でカット3（円軌道）へ接続。
    """
    print("\n  === カット2: 动物横スライド分离 + カメラZ:7→1.0渐变 (AUTO_CLAMPED) ===")

    # 动物はfr216-360で中心位置→分離位置に渐变スライド（既存のease-in-out維持）
    ...
    cam_end = (1.0, -9.0, 1.0)
    _set_camera_location_keyframe(camera, CUT3_START, cam_start)
    _set_camera_location_keyframe(camera, CUT3_END, cam_end)

    # fr360終了時、X=1のままで円軌道へ接続（X=0に戻さない）
    final_cam_cut3 = (1.0, -9.0, 1.0)
    ...
    print(f"  [フレーム {CUT3_END}] carA={separated_pos_a}, carB={separated_pos_b} (スライド分离完了) | カメラZ: 7→1.0, X=1维持")
...
def setup_cut4_orbit(camera, car_a, car_b, separated_pos_a, separated_pos_b, cam_height):
    ...
    軌道パラメータ:
      - 半径: 9m（新仕様準拠）
      - fr360: カット2終了位置 (1, -9, 1.0) から开始
      - 開始角度: atan2(-9, 1) ≈ -83.7° （X=1で円軌道へ接続）
      - 半径補間: fr360→fr376 で √(1²+9²)≈9.06 から 9 へ渐变移行
    ...
    _set_camera_location_keyframe(camera, CUT4_START, (1.0, -9.0, 1.0))
    ...
    final_cam_cut4 = (final_cam_x, final_cam_y, 1.0)

    print(f"  [フレーム {CUT4_END}] 円軌道1周完了（半径9m）")
...
def setup_cut5_return_front(camera, car_a, car_b, separated_pos_a, separated_pos_b, final_cam_cut4, cam_front):
    ...
    print(f"  [フレーム {CUT5_END}] carA={separated_pos_a}, carB={separated_pos_b} (不动) | カメラ={cam_front} (AUTO_CLAMPED自動イージング)")
