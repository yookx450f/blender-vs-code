    ...
    with col_left:
        ...
        with st.form("animal_form", clear_on_submit=False):
            if st.session_state.animal_edit_mode and st.session_state.animal_edit_id:
                ...
                col_dims_a, col_dims_b = st.columns(2)
                with col_dims_a:
                    inp_height = st.number_input("全高 (mm)", min_value=1.0, max_value=100000.0, step=1.0, key="new_animal_height")
                    inp_length = st.number_input("全長 (mm)", min_value=0.0, max_value=100000.0, step=1.0, key="new_animal_length")
