...
- **回転量**: 全フレームで-0.85ラジアン（49.4°）程度（288フレーム基準に比例延長）
- **レンズ**: 35mm（ズームイン設定）
- **ターゲット**: (0.0, 0.0, 1.0) を向く

### 3-3. CarBの半透明化
- **方式**: ドライバー式（Principled BSDFのAlphaに数式ドライバーを追加）
- **フレーム30-120**: スムーズに半透明化 (Alpha: 1.0→0.35)
- 車の重なり部分を強調し、奥行き感を演出

### 3-4. カット1終了地点（fr288時点）
...
## 4. カット2 (fr289-672, 約16秒) — 仕様変更

### 4-1. 概要
- カット1のfr288終了地点から連続して開始
...
| フェーズ | フレーム範囲 | 秒数 | 内容 |
|----------|-------------|------|------|
| フェーズA | fr289 - fr480 | 8秒 (192f) | カメラを x:0, y:0, z:８m まで移動（トップダウンビュー） + 車は中央で静止 |
| フェーズB | fr481 - fr672 | 8秒 (192f) | カメラをカット1開始位置 (-3.0, -6.0, 3.5) までスムーズに戻す + CarB不透明化 + 車をスライドして初期位置へ |

### 4-3. フェーズA (fr289-fr480, 8秒): トップダウン移動 + 車静止
- カメラ: fr288のカメラ位置から (0, 0, 8) へスムーズに移動（イージング適用）
- **車: 中央集合位置で静止**（2台が重なったまま、フェーズBでスライド開始）
- CarB: Alpha=0.35（半透明状態維持）

### 4-4. フェーズB (fr481-fr672, 8秒): カメラ復帰 + 不透明化 + 車スライド
- カメラ: (0, 0, 8) から (-3.0, -6.0, 3.5)（カット1の開始カメラ位置）へスムーズに移動（イージング適用）
- **車: 中央集合位置からカット1開始位置へスライド → fr672で到達**
  - CarA: 中央 → (-1.25, rear_offset_y, grounded_z_a)
  - CarB: 中央 → (1.25, 0.0, grounded_z_b)
- CarB: **fr481で Alpha=1.0 に瞬時に変更**（徐々に戻すのではなく、フェーズB開始と同時に不透明）
  - **重要**: キーフレームの補間モードは `CONSTANT` に設定。fr480(Alpha=0.35)→fr481(Alpha=1.0) の間で滑らかに遷移せず、
    fr480まで Alpha=0.35 が維持されたまま、fr481に到達した瞬間に Alpha=1.0 になる（階段状のステップ変化）。
...
- これにより、fr480までは Alpha=0.35（半透明）が維持され、fr481から突然 Alpha=1.0（完全不透明）になる

### 4-5. カット2終了地点（fr672時点）
...
## 5. カット3 (fr673-912, 約10秒) — 将来実装予定

### 5-1. 概要
- カット2のfr672終了地点から連続して開始
...
- **スタート位置**: (-3.0, -6.0, 3.5)（カット2終了地点のカメラ位置から接続）
...
sequenceDiagram
    ...
    Note over カメラ,CarB: カット1 fr0-288 約12秒
    CarA->>CarA: 右から中央へスライド
    CarB->>CarB: 左から中央へスライド
    CarB->>CarB: Alpha 1.0→0.35 fr30-120
    カメラ->>カメラ: 円弧パンニング 左→右

    Note over カメラ,CarB: カット2 fr289-672 約16秒
    カメラ->>カメラ: フェーズA トップダウンへ移動 (fr289-fr480, 8秒, イージング) + 車は中央で静止
    カメラ->>カメラ: フェーズB 開始位置へ戻る (fr481-fr672, 8秒, イージング)
    CarB->>CarB: Alpha=0.35維持 (フェーズA中)
    CarB->>CarB: Alpha→1.0 瞬時不透明化 (fr481でフェーズB開始)
    CarA->>CarA: fr481-fr672で中央からスタート位置へスライド (イージング)
    CarB->>CarB: fr481-fr672で中央からスタート位置へスライド (イージング)

    Note over カメラ,CarB: カット3 fr673-912 約10秒 (将来実装予定)
    カメラ->>カメラ: 円弧パンニング 左→右
    CarA->>CarA: スタート位置で静止
    CarB->>CarB: スタート位置で静止
```
...
|--------|-------------|------|-----------|---------|-----------|
| カット1 | fr0 - fr288 | ~12秒 | 円弧パンニング(左→右) | 中央へスライド後静止 | Alpha 1.0→0.35 |
| カット2A | fr289 - fr480 | **8秒** | トップダウンへ移動 (z:8m) [イージング] | 中央集合位置で静止 | Alpha 0.35維持 |
| カット2B | fr481 - fr672 | **8秒** | 開始位置へ復帰 [イージング] | 中央からスタート位置へスライド | Alpha→1.0 (fr481で瞬時) |
| カット3 | fr673 - fr912 | ~10秒 | 円弧パンニング(左→右) | 静止 | Alpha 1.0維持 |
