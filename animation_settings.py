...
def setup_all_animations(scene, camera, imported_cars, rear_offset_y, grounded_z_positions, car_dimensions=None):
    ...
    #     シーン 5:   真横固定視点・全長差表示エフェクト（5 秒）
    #         停止（2 秒）
    #     シーン 6:    カメラ移動・正面ビューへ（全長差表示フェードアウトしてゆっくり消す）（7 秒）
    #         停止（2 秒）
    scene.frame_start = 0
    scene.frame_end = 1080
    scene.render.fps = 24
    ...
    print(f"              停止（2 秒）")
    print(f"  【カット 2】シーン 6（フレーム 768-936):")
    print(f"              カメラ移動・正面ビューへ（全長差表示フェードアウトしてゆっくり消す）（7 秒）")
    print(f"              停止（2 秒）")
    print(f"  【カット 2】シーン 7（フレーム 936-1080):")
    print(f"              カメラ固定・正面ビュー（6 秒）")
