"""
アニメーション設定モジュール - ショート動画v1（縦長9:16）
フレ ーム 0-288（約12秒、24fps）を処理する。

カット1 (fr0-288): 「車が重なっていく部分」の円弧パンニング

Short2と違い、カット1のみで構成され、前半（車スライド+円弧パンニング）だけで完結する。

YouTube Shorts用の縦長フォーマット。

short1 の特徴:
    Short2と同じアニメーションロジックを使用（cut1のみ）。
    半透明化はShort2と同じ挙動（fr30で半透明化、終了フレームで不透明化）。
...
使い方:
    from animation_settings_short1 import setup_short1_animations
    setup_short1_animations(scene, camera, imported_cars, rear_offset_y, grounded_z_positions, strategy_config=None, car_dimensions=None)
"""
...
def setup_short1_animations(scene, camera, imported_cars, rear_offset_y, grounded_z_positions, strategy_config=None, car_dimensions=None):
    """
    ショート動画v1のアニメーションをオーケストレーション（フレーム 0-288）
    ...
    Returns:
        CutState: 最終状態情報
    """
    print(f"\n=== ショート動画v1 アニメーション設定を開始 (total_frames=288, 約12秒) ===")
    ...
    # ============================================================
    # 総フレーム数はカット1のみ (fr0-288)
    # ============================================================
    total_frames = 288  # 約12秒
    ...
    # キーフレームの位置を総フレームに合わせて調整（終了フレームで不透明化）
    alpha_restore_frame = total_frames
    ...
    print(f"  carA: {car_a_end}")
    print(f"  carB: {car_b_end}")
    ...
    print(f"\n=== ショート動画v1 アニメーション完了 (total_frames={total_frames}, 約{total_frames/24:.1f}秒) ===")
