...
仕様書 (plans/shortAnimal仕様.md) に基づく5段階のカット構造:
  カット1 (fr0-72,   3秒):    重叠状態、動物はすべて不透明。カメラ位置X:1m Y:-8m Z:0.5mで静止。視点（ターゲット）が斜め上20度から動物Bの最高部へゆっくり移動
  カット1-2 (fr72-216, 6秒): 重叠状態維持、動物Bが半透明(瞬時0.35)。カメラ位置Y:-8→-9 + Z:0.5→7.0平滑移動（中間点Y:-8.5,Z:2.75経由）。視点(ターゲット)は動物Bの最高部からmax(A,B)*0.75へ移動
  カット2 (fr216-360, 6秒):  横向きスライドで2体が瞬時分離。不透明化完了（瞬時）。カメラ位置(X:1m,Y:-9m,Z:7→1m)平滑下降、その後円軌道へ平滑接続
  カット3 (fr360-960, 25秒): 半径9m円軌道で1周（両方の動物を視界を入れながら）
  カット4 (fr960-1032, 3秒): カメラはゆっくり正面(X:2m,Y:-5m)に戻る、動物は動かさない
...
def setup_shortAnimal_animations(scene, camera, imported_cars, rear_offset_y, grounded_z_positions, car_dimensions=None, total_frames=1032):
    ...
    仕様書に基づく5段階のカット構造:
      カット1 (fr0-72,   3秒):    重叠状態、すべて不透明。カメラX:1m Y:-8m Z:0.5mで静止。視点(ターゲット)が斜め上20度から動物Bの最高部へゆっくり移動
      カット1-2 (fr72-216, 6秒):  重叠状態維持、动物B半透明(瞬時0.35)。カメラY:-8→-9 + Z:0.5→7平滑移動（中間点経由）
      カット2 (fr216-360, 6秒):  横向きスライドで2体が瞬時分離。不透明化完了（瞬時）。カメラ位置(X:1m,Y:-9m,Z:7→1m)平滑下降、その後円軌道へ平滑接続
      カット3 (fr360-960, 25秒): 半径9m円軌道で1周（両方の動物を視界に入れながら）
    ...
    # カメラ位置定義（仕様に準拠）
    # カット1/カット1-2: Y=-8, Z=0.5m; カット2: Y=-9, Z=1m; カット4: Y=-5に戻り
    cam_fixed_cut1 = (1.0, -8.0, 0.5)      # カット1・1-2: 正面、Y=-8m, Z=0.5m
    ...
    print(f"    カット3: fr360-960 ({360/24:.1f}-{960/24:.1f}秒) 半径9m円軌道")
    ...
    # カット1-2: 半透明化 + 左右スライド分離 (カメラY:-8→-9, Z:0.5→7.0、中間点経由)
